...
const checkIsThisParamsValide = (matrix: Matrix) => {
  if (!isValideParamsForApp(matrix)) {
    throw new InvalideParamsForAppError("Your params are not valide (according to the autor)")
  }
}
/**
...
const isValideParamsForApp = (matrix: Matrix): boolean => {
  if (valideParamsForApp.rowMoreThen && matrix.rows < valideParamsForApp.rowMoreThen) {
    return false;
  }
  if (valideParamsForApp.colunMoreThen && matrix.columns < valideParamsForApp.colunMoreThen) {
    return false;
  }
  if (valideParamsForApp.rowLessThen && matrix.rows > valideParamsForApp.rowLessThen) {
    return false;
  }
  if (valideParamsForApp.colunLessThen && matrix.columns > valideParamsForApp.colunLessThen) {
    return false;
  }
